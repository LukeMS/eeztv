...
class EztvIt(object):
    ...
    def get_shows(self):
        ...
        logger.info("EztvIt: getting shows...")

        javascript_source = self._get_shows_dropdown_javascript().decode('utf-8')
        ...
        for parsed_show in parsed_shows:
            ...
            if match:
                ...
                name_parts = [_f for _f in [match.group(2), match.group(1), match.group(3)] if _f]
    ...
    def get_episodes(self, show_name):
        ...
        match = [candidate for candidate in list(shows.values()) if candidate.lower().startswith(show_name.lower())]
    ...
    def get_episodes_by_id(self, show_id):
        ...
        for anchor in release_anchors:
            ...
            if torrent_links:
                ...
                links['torrents'] = [urllib.parse.urlparse(href, SCHEME).geturl() for href in hrefs]

            # Find the anchor that looks like it has a title for the filesize.
            filesize_regex = re.compile(r'(\d+\.?\d+) (MB|GB|B)')
