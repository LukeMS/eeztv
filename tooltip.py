...
- anchor: where the text should be positioned inside the widget, must be on of
"n", "s", "e", "w", "nw" and so on. Default is "center";
- bd: borderwidth of the widget; default is 1 (NOTE: don't use "borderwidth"
here);
- bg: background color to use for the widget; default is "lightyellow" (NOTE:
don't use "background");
- delay: time in ms that it takes for the widget to appear on the screen when
the mouse pointer has entered the parent widget; default is 1500;
- fg: foreground (i.e. text) color to use; default is "black" (NOTE: don't use
"foreground");
...
- justify: how multiple lines of text will be aligned, must be "left", "right"
...
Default is "solid";
- state: must be "normal" or "disabled"; if set to "disabled" the tooltip will
not appear; default is "normal";
- text: the text that is displayed inside the widget;
- textvariable: if set to an instance of tkinter.StringVar() the variable's
value will be used as text for the widget;
- width: width of the widget; the default is 0, which means that "wraplength"
...
- motion(): is called when the mouse pointer moves inside the parent widget if
follow_mouse is set to 1 and the tooltip has shown up to continually update
the coordinates of the tooltip window;
...
class ToolTip:
    ...
    def __init__(self, master, text='Your text here', delay=750, **opts):
        self.master = master
        self._opts = DEFAULT_OPTIONS.copy()
        self._opts['delay'] = delay
        self._text = tk.StringVar(master)

        self.text = text
        self._opts['textvariable'] = self.text
...
def set_tooltip(master, *args, **kwargs):
    master.tooltip = ToolTip(master, *args, **kwargs)
...
if __name__ == '__main__':
    ...
    l.pack(side='top')
    set_tooltip(l, follow_mouse=1, text=(
        "I'm a tooltip with follow_mouse set to 1.\n"
        "I won't be placed outside my parent!"))
    b = tk.Button(root, text='Quit', command=root.quit)
    b.pack(side='bottom')
    set_tooltip(b, text='Enough of this')
    root.mainloop()
